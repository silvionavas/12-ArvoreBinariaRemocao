...
void removerElementoArvore(NO* no, int valor) {
    ...
    if (atual->esq == NULL && atual->dir == NULL) {
        if (pai == NULL) {  // Se for o nó raiz
            no = NULL;  // A árvore fica vazia
        } else if (pai->esq == atual) {  // Se for o filho esquerdo do pai
            pai->esq = NULL;
        } else {  // Se for o filho direito do pai
            pai->dir = NULL;
        }
        free(atual);
    }
    ...
    // Caso 3: Nó com dois filhos
        ...
        // Encontrar o sucessor (menor nó na subárvore à direita)
